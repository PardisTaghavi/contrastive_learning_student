...
class BasePixelDecoder(nn.Module):
    ...
    def forward_features(self, features):
        ...
        print("multi_scale_features", len(multi_scale_features))
        for i in range(len(multi_scale_features)):
            print(multi_scale_features[i].shape, "multi_scale_features of ", i)

        print("y", y.shape)
        print("mask_features", self.mask_features(y).shape)
        breakpoint()
...
class TransformerEncoderOnly(nn.Module):
    ...
    def forward(self, src, mask, pos_embed):
        ...
        print("-------> are we in transformer encoder only")
        bs, c, h, w = src.shape
...
class TransformerEncoderPixelDecoder(BasePixelDecoder):
    ...
    def forward_features(self, features):
        print("-------> are we in transformer encoder pixel decoder")
        multi_scale_features = []
        num_cur_levels = 0
        # Reverse feature maps into top-down order (from low to high resolution)
        for idx, f in enumerate(self.in_features[::-1]):
